...
namespace Interrapidisimo\Credibanco\Model\Method;

/**
 * Checkout Payment Method Model Class
 * Class Checkout
 * @package Interrapidisimo\Credibanco\Model\Method
 */
class Checkout extends \Magento\Payment\Model\Method\AbstractMethod {

    use \Interrapidisimo\Credibanco\Model\Traits\OnlinePaymentMethod;

    const CODE = 'credibanco_checkout';
    ...
    protected function checkout($data) {
        ...
        $request = [
            "orderNumber" => (string)\BeGateway\Settings::$prefijo.$data['order']['real_id'],
            "amount" => $data['order']['amount'],
            "returnUrl" => \BeGateway\Settings::$domainReturn.'/'.openssl_encrypt($data['order']['real_id'], \BeGateway\Settings::$ciphering, \BeGateway\Settings::$encryptionKey, 0, \BeGateway\Settings::$cryptionV),
            "description" => 'Order # ' . $data['order']['increment_id'],
            "ip_address" => $this->getConfigHelper()->getUserIP(),
        ];
        
        $response = $transaction->submit($request);
    ...
    public function order(\Magento\Payment\Model\InfoInterface $payment, $amount) {
        ...
        try {
            ...
            $isBeGatewaySuccessful = $responseObject->getStatus() == 'success' && isset($responseObject->getMessage()->formUrl) && !empty($responseObject->getMessage()->formUrl);
            ...
            $payment->setTransactionId(\BeGateway\Settings::$prefijo . $order->getIncrementId());
            $payment->setParentTransactionId($responseObject->getToken());
     ...
     * @return bool
     */
    /* 1. Valida si existe un carrito disponible */
     ...
     * @return bool
     */
    /* 2. Valida los tipos de monedas disponibles */
